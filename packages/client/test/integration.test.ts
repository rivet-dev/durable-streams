...
describe(`Stream Close Operations`, () => {
  testWithStream(
    ...
    async ({ streamUrl, store, streamPath, aborter }) => {
      ...
      // Verify the stream is closed in the store
      const storedStream = store.get(streamPath)
      expect(storedStream?.closed).toBe(true)
    }
  )

  testWithStream(
    `should allow idempotent close`,
    async ({ streamUrl, store, streamPath, aborter }) => {
      const stream = new DurableStream({
        url: streamUrl,
        signal: aborter.signal,
      ...
      // Close the stream twice - should not error
      await stream.close()
      await stream.close()
  ...
  testWithStream(
    `should reject appends to closed stream`,
    async ({ streamUrl, store, streamPath, aborter }) => {
      // Close the stream first
      store.closeStream(streamPath)
      ...
      // Append should fail with 409 Conflict
      await expect(stream.append(`should fail`)).rejects.toThrow(FetchError)
    }
  )

  testWithStream(
    `should include closed status in head()`,
    async ({ streamUrl, store, streamPath, aborter }) => {
      // Close the stream first
      store.closeStream(streamPath)
  ...
  testWithStream(
    `should include closed status in read()`,
    async ({ streamUrl, store, streamPath, aborter }) => {
      // Append some data then close
      ...
      const result = await stream.read()
      expect(decode(result.data)).toBe(`some data`)
      expect(result.closed).toBe(true)
    }
  )

  testWithStream(
    `should stop following when stream is closed`,
    async ({ streamUrl, store, streamPath, aborter }) => {
      const stream = new DurableStream({
        url: streamUrl,
        signal: aborter.signal,
      ...
      const chunks: Array<{ data: string; closed?: boolean }> = []
      ...
      // Append some data then close
      store.append(streamPath, encode(`final data`))
      store.closeStream(streamPath)

      await followPromise

      // Should have received the data and then stopped when closed
      const dataChunks = chunks.filter((c) => c.data.length > 0)
      expect(dataChunks.some((c) => c.data === `final data`)).toBe(true)

      // Final chunk should have closed flag
      const lastChunk = chunks[chunks.length - 1]
      expect(lastChunk?.closed).toBe(true)
    }
  )

  testWithStream(
    `should not wait in long-poll for closed stream`,
    async ({ streamUrl, store, streamPath, aborter }) => {
      // Append data and close the stream
      ...
      // Read should return immediately with closed flag
  ...
  testWithStream(
    `should complete follow iterator after reading all data from closed stream`,
    async ({ streamUrl, store, streamPath, aborter }) => {
      // Setup: append data and close
      store.append(streamPath, encode(`chunk1`))
      store.append(streamPath, encode(`chunk2`))
      store.closeStream(streamPath)
      ...
      // Follow should complete naturally
      for await (const chunk of stream.follow({
        live: `catchup`,
        signal: aborter.signal,
      })) {
        ...
        if (chunk.upToDate && chunk.closed) {
          break
        }
      }
      iteratorCompleted = true

      // Data may be concatenated in one chunk
